...
export const FIVERR_PROFILE_RULES = [
  ...
  "- PLAIN TEXT ONLY — no markdown asterisks (** or *), no # headings, no backticks.",
  "- Mention niches and tools buyers recognize. Invite them to view your gigs / send a brief.",
  "- NEVER invent clients, companies, or ratings not in the scraped profile.",
  "",
  "SKILLS:",
 ...
 *
 * Also optimized for Uma Recruiter (Upwork’s AI shortlist agent): profiles are
 * scored as machine-readable corpora — niche density + explicit nouns +
 * consistent signals across title / overview / skills / recent work beat
 * broad stack lists. Upwork’s engineering blog confirms Uma weighs profile
 * description, expertise, past performance, portfolio, availability, JSS.
 */
export const UPWORK_PROFILE_RULES = [
  "MISSION: Make the Upwork profile win Uma shortlists + human clicks → invites → interviews.",
  "Write for TWO readers: (1) Upwork/Uma AI matching, (2) a stressed client skimming search results.",
  "",
  "NICHE DENSITY (critical for 2026 matching):",
  "- Pick ONE primary vertical + stack combo the AI can confidently pattern-match",
  "  (e.g. 'SaaS payments/billing (Stripe Connect)', 'Shopify performance', 'Next.js B2B SaaS') — not 6 industries at once.",
  "- Repeat that niche language consistently in: headline, first 2 overview sentences, skills, and portfolio framing.",
  "- Scattered signals (dev + design + VA, or SaaS+CRM+healthcare+fintech+education with equal weight) dilute match confidence.",
  "- Secondary skills can appear lower in the overview; the ABOVE-THE-FOLD must be unambiguous.",
  "",
  "TITLE (headline) — hard limit 70 characters:",
  "- Outcome or specialization first — NOT stack soup.",
  "- BAD: 'Full Stack Web Developer | Next.js, React, Node.js, Supabase & Laravel' (generic, every other profile).",
  "- BAD: Restating the job title with no new information.",
  "- GOOD: niche + problem/outcome, e.g. 'SaaS Billing & Stripe Connect Developer | Next.js'",
  "  or 'I turn slow Shopify stores into fast ones' (when true to scrape).",
  "- Use nearly all 70 chars. Prefer client-search nouns over soft adjectives.",
  "",
  "OVERVIEW — Upwork allows 5,000 characters; TARGET 2,000–3,500 characters:",
  "- First 200–250 characters are the ONLY lines in search before Read more — AND heavily weighted by matching.",
  "- FORBIDDEN openings: 'Hi, my name is…', 'I am a passionate…', 'Full-Stack Developer specializing in…' (title repeat).",
  "- First TWO sentences = 100% result / pain focused (what you fix for whom). Put proof early, not after a bio.",
  "- PLAIN TEXT ONLY. No markdown. Forbidden: **, *, # headings, `backticks`, [links](url).",
  "- Short paragraphs separated by blank lines. Prefer prose. Hyphen lists ('- item') only if needed — never asterisks.",
  "- Structure (conversion-tested order):",
  "  1) Two killer result-driven opening sentences (search snippet)",
  "  2) Who you're a good fit for (their specific struggles)",
  "  3) Benefits / business outcomes of working with you (not jargon dumps)",
  "  4) Who's a bad fit (builds credibility — optional but powerful when natural)",
  "  5) How you work (process, communication, timelines)",
  "  6) Proof — ONLY clients/people/companies/metrics in the scrape. If none, capability + process without fake names.",
  "  7) Short about-me LAST — not the star of the show",
  "  8) Soft CTA (message with brief / timeline)",
  "- Peer tone for mobile skim. No contact info (email/phone/Skype).",
  "",
  "PORTFOLIO / PROOF GUIDANCE (in critique + action plan):",
  "- Prefer 3 excellent case studies over 10+ vague screenshots or GitHub dumps.",
  "- Frame as: problem → what you built → measurable result (load time, conversion, $ processed, time saved).",
  "- Flag vague results like 'improved productivity' with no numbers.",
  "- Employment history: if scrape shows many thin/templated overlapping roles, recommend trimming to 3–4 verifiable niche-aligned roles (do not invent history).",
  "",
  "VIDEO INTRO:",
  "- Do not prioritize video intro in the action plan — low ROI vs headline/overview/skills/portfolio.",
  "",
  "TRUTH RULES (critical — violating damages the seller):",
  "- NEVER invent clients, companies, countries, earnings, Job Success %, ratings, or tools not in the scrape.",
  "- If Named clients list is empty, do not invent 'worked with X in Germany/Dubai/US' style claims.",
  "- Prefer keeping real names from the scrape over generic marketing fluff.",
  "- Never copy another freelancer's phrasing; extract patterns only.",
  "",
  "HOURLY RATE (stage-aware × niche — critical):",
  "- First classify the account from the scrape: getting_started | building_proof | established.",
  "  getting_started = few/no jobs, thin portfolio, no named clients, short/weak overview, or clearly struggling for work.",
  "  building_proof = some reviews/portfolio but not yet premium-positioned.",
  "  established = strong proof, many clients/reviews, clear seniority.",
  "- getting_started: NEVER suggest premium rates. Use THIS niche's entry band (writers lower than web/dev; email/SEO mid; AI entry still higher than writing but NOT $95+).",
  "  Goal = remove hire-friction and win first 3–5 reviews. Focus action plan on standing out for THAT niche.",
  "- building_proof: mid-market for THAT niche after a few solid reviews.",
  "- established: then suggest market/premium for THAT niche only (e.g. payments/SaaS infra can sit above generic full-stack).",
  "- Rate note must name the niche logic. Never tell a struggling seller to charge like a Top Rated expert.",
  "- They can always bid differently per job; profile rate should match niche + traction TODAY.",
  "",
  "SKILLS (machine-readable matching input):",
  "- Prefer specific client-search tags over generic ones ('Next.js', 'Stripe Connect', 'Shopify Liquid' > 'Web Development', 'JavaScript').",
  "- Keep 10–15 tightly related skills in ONE coherent service offering — not unrelated categories.",
  "- Prefer skills already on the profile or clearly implied — do not invent unrelated stacks.",
  "",
  "CRITIQUE must actively flag when present:",
  "- Greeting / name openers wasting above-the-fold space",
  "- Headline or first sentence that only restates the title / stack list",
  "- Stack-soup headlines with no outcome or niche",
  "- Too many equal-weight verticals (broad generalist)",
  "- Grammar / sloppy copy that signals sloppy work",
  "- Vague portfolio outcomes without metrics",
  "- Bloated or templated employment history that risks credibility",
  "",
  "ACTION PLAN:",
  "- Numbered steps: rewrite headline → paste overview (snippet-first) → set niche skills → trim/reframe 3 case studies → set stage-appropriate rate → (optional) prune employment history.",
  "- Each step: short title + one concrete detail (where to click / what to paste).",
  "- Include a 'beta test' step: search your niche as a client and check the search snippet cuts cleanly.",
  "",
  `- Ban fluff: ${BANNED_FLUFF}.`,
].join("\n")
